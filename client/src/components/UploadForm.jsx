...
const BACKEND_URL = "http://10.204.236.30:10000";
const MAX_RETRIES = 3;
  ...
  const loadRelatedImages = (className) => {
    try {
      const context = require.context(
        ...
        false,
        new RegExp(`^./<span class="math-inline">\{className\}/\.\*\\\\\.jpg</span>`)
      );
  ...
  const uploadFiles = async () => {
    ...
    try {
      ...
      if (response.data.segmented_image) {
        const segmentedImageUrl = `<span class="math-inline">\{BACKEND\_URL\}</span>{response.data.segmented_image}`;
      ...
      if (err.code === "ECONNABORTED") {
        if (retries < MAX_RETRIES) {
          ...
          setError(`Request timed out. Retrying (<span class="math-inline">\{retries \+ 1\}/</span>{MAX_RETRIES})...`);
